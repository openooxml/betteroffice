...
/** A value an overflow action asks for in a dialog, such as a custom font size. */
...
/** A color an overflow action asks for with the platform color picker. */
export interface OverflowColorRequest {
  value: string;
...
export interface OverflowRegistry {
  ...
  /** Opens the platform color picker; call during the activating input event. */
  pickColor(request: OverflowColorRequest): void;
  /** Element popups opened from the menu render into, inside the toolbar. */
  popupHost(): HTMLElement | null;
  /** Moves focus to the menu button, or the toolbar when nothing overflows. */
  focusMore(): void;
}
