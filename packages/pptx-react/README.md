...
- **Lifecycle.** Once a presentation is open, each plugin gets fresh state,
  ...
  plugin added to an open presentation), and its contributions appear. It then
  receives `document-change` (the committed version only, for typing, commands,
  batches, undo, redo and remote updates, never for refusals or no-ops),
  `selection-change`, `mode-change` (with `readOnly`), `layout-change` and
  `grants-change`. Events describe current state: several changes may arrive as
  one, and a newer one aborts the hook still handling the previous
  (`context.signal`). Replacing the presentation, removing the plugin, changing
  its `revision`, unmounting, or a failure ends the activation: its signals
  abort, its clients refuse, and every `onCleanup` disposer runs once with the
  reason. Plugins are matched by `id` and `revision`, so new array or callback
  identities and reordering keep their state. `context.run(action)` gives event
  handlers a fresh context and isolates their failures. `onReady` is unaffected
  by plugins.
