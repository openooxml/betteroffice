...
import type { PptxPlugin, PptxPluginCommand, PptxPluginDefinition } from './types';

const definitions = new WeakMap<object, PptxPluginDefinition<unknown>>();
...
function frozenCommand<S>(command: PptxPluginCommand<S>): PptxPluginCommand<S> {
...
export function definePptxPlugin<S>(definition: PptxPluginDefinition<S>): PptxPlugin {
  ...
  definitions.set(plugin, frozen as PptxPluginDefinition<unknown>);
  return plugin as unknown as PptxPlugin;
}

/** The definition behind a plugin this package created, or null. */
export function pluginDefinition(plugin: unknown): PptxPluginDefinition<unknown> | null {
...
export function definitionProblem(definition: PptxPluginDefinition<unknown>): Error | null {
