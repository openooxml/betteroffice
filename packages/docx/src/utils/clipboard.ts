...
function getDomPurify(): ReturnType<typeof createDOMPurify> {
  // dompurify's WindowLike picks DOM constructors off `typeof globalThis`, which
  // the d.ts build resolves against a lib set where `window` doesn't satisfy the
  // pick structurally; a live window always carries these globals at runtime.
  return (domPurify ??= createDOMPurify(window as unknown as WindowLike));
}
