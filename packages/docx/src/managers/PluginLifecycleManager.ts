...
export class PluginLifecycleManager extends Subscribable<PluginLifecycleSnapshot> {
  ...
  /**
   * Initialize plugins with the adapter's current editor state.
   * Calls `plugin.initialize(editorState)` for each plugin.
   *
   * Note: CSS injection and DOM event listeners are the responsibility
   * of the framework-specific host (e.g. React PluginHost).
   */
  initialize(plugins: PluginLifecycleConfig[], editorState: unknown): void {
    // Clean up previous
    this.destroyPlugins();
    ...
    for (const plugin of plugins) {
      if (plugin.initialize && !this.pluginStates.has(plugin.id)) {
        this.pluginStates.set(plugin.id, plugin.initialize(editorState));
      }
    }
   ...
   */
  updateStates(editorState: unknown): boolean {
    let anyChanged = false;
    for (const plugin of this.plugins) {
      if (plugin.onStateChange) {
        const newState = plugin.onStateChange(editorState);
