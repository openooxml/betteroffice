...
export function TablePropertiesDialog({
  ...
  const [width, setWidth] = useState<number>(Number(currentProps?.width) || 0);
  const [widthType, setWidthType] = useState<string>(String(currentProps?.widthType || 'auto'));
  const [justification, setJustification] = useState<string>(
    String(currentProps?.justification || 'left')
  );

  useEffect(() => {
    if (isOpen) {
      setWidth(Number(currentProps?.width) || 0);
      setWidthType(String(currentProps?.widthType || 'auto'));
      setJustification(String(currentProps?.justification || 'left'));
    }
  }, [isOpen, currentProps]);
