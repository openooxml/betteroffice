...
fn shape_run_formatting(source: Option<&Value>) -> RunFormatting {
    ...
    serde_json::to_string(&Value::Object(output)).ok().and_then(|s| serde_json::from_str(&s).ok()).unwrap_or_default()
}
...
fn shared_plus_geometry(shape_type: &str, shape: &Value) -> Option<Vec<Value>> {
    ...
    path.into_iter()
        .map(|p| serde_json::to_string(&p).ok().and_then(|s| serde_json::from_str::<Value>(&s).ok()).ok_or(()))
