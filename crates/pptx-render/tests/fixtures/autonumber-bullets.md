...
`crates/pptx-edit/tests/fixtures/deck-schema-v9-autonumber.update.bin` was generated by main `2c90c17f` using `cargo run -p betteroffice-pptx-edit --example schema_fixture -- seed <this-deck> <output>`. It migrates once to v10; attaching the source imports restart flags into both the parsed package and surviving story paragraphs. Source-free v9 snapshots cannot recover explicit `startAt="1"` until that source is attached.
