    ...
    NoteLayoutItemContract {
        ...
        blocks: Some(
            content
                .blocks
                .iter()
                .filter_map(|block| serde_json::to_string(&block).and_then(|s| serde_json::from_str::<serde_json::Value>(&s)).ok())
                .collect(),
        ),
        measures: Some(
            content
                .measures
                .iter()
                .filter_map(|measure| serde_json::to_string(&measure).and_then(|s| serde_json::from_str::<serde_json::Value>(&s)).ok())
                .collect(),
        ),
