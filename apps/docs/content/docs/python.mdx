---
title: Python
description: betteroffice-docx, betteroffice-xlsx, and betteroffice-pptx on PyPI — read, edit, lay out, render, and save
---
...
All three formats are on PyPI. The Rust engine is compiled into each wheel: no
Word, no Excel, no PowerPoint, no LibreOffice subprocess, no COM. Wheels are
built for Linux (x86_64, aarch64), macOS (arm64, x86_64), and Windows (x86_64)
against the stable ABI for CPython 3.9 and up.

[`betteroffice-xlsx`](https://pypi.org/project/betteroffice-xlsx/) is the
workbook distribution, and the rest of this page walks through it;
[DOCX](#docx) and [PPTX](#pptx) follow at the end.
...
edit keeps its unmodeled row, column, and cell markup: only the cells, rows, and
columns the edit actually changed are rewritten, and the rest of the sheet is
kept byte for byte. A sheet whose rows or cells lack `r` attributes or arrive out of
order, or that was replayed from collaboration updates, is reserialized from the
model instead.
...
paragraphs, tables, and sections, rewrites the text of
a plain single-run paragraph, and saves back to DOCX with everything the model
does not represent reused from the package it opened. It paginates and
rasterizes too: the layout pass takes an already-measured projection of the
document, because the Rust crates do not lower the DOCX model into measured
blocks yet, and `render_png` draws one page of the resulting display list. No
font is compiled into that wheel, and a page whose text names a family you never
registered is an error rather than a page of invisible text.
...
that wheel. `render_png` rasterizes a slide to PNG bytes from that same layout,
resolving pictures out of the package, so only fonts need registering. A `Presentation` is also pinned to the thread that opened it, and
must be released there.

Comments read and write in both PowerPoint formats. `comments()` returns every
comment on the deck and `comment_flavor` reports which system it uses: `legacy`
...
A file commits to one at its first comment and never carries both, so
`set_comment_flavor` only works while the deck has no comments, and
`reply_to_comment` and `set_comment_status` raise on a legacy deck.
...
Positions are EMU, like every other coordinate in the API. `created` is
caller-supplied rather than read from a clock, so the same edits replay to the
same bytes.
