...
`initialize` and one `load` event per workbook (repeated when it changes before
that hook finishes), then coalesced `document-change`, `selection-change`, `mode-change` (with `readOnly`),
