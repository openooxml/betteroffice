...
Every format ships browser packages under the `@betteroffice` scope: a
framework-free core, a React editor built on it, and that editor's locale
strings. DOCX, XLSX, and PPTX also ship a `-native` package that calls the native
Rust facade. There is no Vue package — React, framework-free JavaScript, and
native Node.js and Bun are the supported entry points.
