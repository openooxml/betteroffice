...
carries an anchor (`text` ranges use `readContent()` offsets) and, when read
from the file, its source part, SHA-256 and element path. Hidden slides and
shapes, notes and comments are explicit options, layout and master content is
not exported, and every omission is a diagnostic. `maxBlocks` and `maxBytes`
stop the export at a whole record and set `truncated`. Markdown marks each
