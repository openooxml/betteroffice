...
PPTX comments read and write in both PowerPoint formats. `comment_flavor`
reports which system a deck uses — `CommentFlavor::Legacy` reads everywhere,
`CommentFlavor::Modern` carries replies and a resolved state but only shows in
PowerPoint 365. A file commits to one at its first comment and never carries
both, so `set_comment_flavor` only works while the deck has no comments, and
...
presentation.add_comment(
    ...
    1_828_800, // EMU, like every other coordinate
    914_400,
)?;
```

The timestamp is caller-supplied rather than read from a clock, so the engine
stays clock-free and the same edits replay to the same bytes on every peer.
...
PPTX keeps its rasterizer behind a feature, for the same reason DOCX does:

```bash
cargo add betteroffice-pptx --features raster
```

`render_slide` returns the display list the browser editor paints — vector
geometry, images, shaped text, caret data — alongside hit-test metadata, and it
fails until at least one font face has been registered. With the feature on,
`render_png` rasterizes a slide straight from the deck, resolving pictures out of
the package, so only fonts need registering. `save` writes Yrs edits back into
PresentationML: parts an edit did not touch keep their exact source bytes, and
edited slides are patched at the XML level so unmodeled markup survives. The ZIP
container is rebuilt, so the output is not byte-identical to the input.
...
| `betteroffice-pptx-raster` | The tiny-skia backend that paints a PPTX display list to PNG. Never part of the wasm build. |
