...
the package, so only fonts need registering. Yrs edits reach that display list
but are not projected back into PresentationML, so `save` re-zips the parts it
retained rather than your edits.
