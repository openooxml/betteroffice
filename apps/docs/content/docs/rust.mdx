...
The export reads edits made through the model, lists omitted and unsupported
...
bytes. Page fragments are not included yet.
...
keeps the exact matches of an id, tag or alias:
...
Ids and anchors address the returned snapshot. A live `EditingDoc` lists the same
