...
npm and crates.io carry all three formats. PyPI carries `betteroffice-docx` and
`betteroffice-xlsx`; the PPTX binding is built and tested in the repository but
has no PyPI project yet.
...
- **A server, a notebook, or an agent in Python** — `betteroffice-docx` and
  `betteroffice-xlsx`.
