...
Both font packages ship built ESM rather than TypeScript sources. Node refuses to type-strip files under `node_modules`, so a source-published package is unimportable from plain Node — the server-side host this work exists to serve. For the same reason the loader now reads `file:` assets from disk: Node's `fetch` does not implement the `file:` scheme.
...
When no provider resolves and native measurement becomes unsupported, the registry now warns once with what to install and explains that browser fallback may use OS fonts, instead of silently paginating differently.
