...
- The resident caret painted at the paragraph's `pm_start` plus the raw `Loc` offset, which placed it one glyph past the caret for a paragraph sitting directly behind a block embed. That offset is now rebased onto the paragraph's display node before the rect is resolved, so the painted caret and the insertion point agree.
