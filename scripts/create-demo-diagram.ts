...
function geometry(): string {
  return `<Section N='Geometry' IX='0'><Row T='MoveTo' IX='0'>${cells({ X: "0", Y: "0" })}</Row><Row T='LineTo' IX='1'>${cells({ X: "1", Y: "0" })}</Row><Row T='LineTo' IX='2'>${cells({ X: "1", Y: "1" })}</Row><Row T='LineTo' IX='3'>${cells({ X: "0", Y: "1" })}</Row><Row T='LineTo' IX='4'>${cells({ X: "0", Y: "0" })}</Row></Section>`;
}

function shape(id: number, name: string, x: number, y: number, width: number, height: number, fill: number, text: string): string {
  return `<Shape ID='${id}' Name='${name}' NameU='${name}' Type='Shape'>${cells({ PinX: String(x), PinY: String(y), Width: String(width), Height: String(height), LocPinX: String(width / 2), LocPinY: String(height / 2), FillForegnd: String(fill), LineColor: "2", LineWeight: "0.018", ThemeIndex: "0" })}${geometry()}<Text>${text}</Text></Shape>`;
}

function connector(): string {
  return `<Shape ID='30' Name='Release connector' NameU='Release connector' Type='Shape'>${cells({ PinX: "5", PinY: "4.25", Width: "3.5", Height: "0.1", BeginX: "3.25", BeginY: "4.25", EndX: "6.75", EndY: "4.25", LineColor: "2", LineWeight: "0.028" })}<Section N='Geometry' IX='0'><Row T='MoveTo' IX='0'>${cells({ X: "0", Y: "0" })}</Row><Row T='LineTo' IX='1'>${cells({ X: "1", Y: "0" })}</Row></Section></Shape>`;
}
...
const page1 = `<PageContents xmlns='http://schemas.microsoft.com/office/visio/2012/main'><Shapes>${shape(1, "BetterOffice", 2.25, 5.5, 3.5, 1.15, 1, "BETTEROFFICE")}<Shape ID='10' Name='Capability group' NameU='Capability group' Type='Group'>${cells({ PinX: "6.8", PinY: "3.7", Width: "4.8", Height: "2.8", LocPinX: "2.4", LocPinY: "1.4" })}<Shapes>${shape(11, "Read", 5.6, 3.7, 1.6, 1.1, 3, "READ")}${shape(12, "Edit", 7.4, 3.7, 1.6, 1.1, 4, "EDIT")}</Shapes></Shape>${shape(20, "Rust engine", 5, 1.65, 5.6, 1.15, 1, "RUST ENGINE")}</Shapes></PageContents>`;
