...
export const pythonRoundtrip: DocxScenario = {
  ...
  async run(ctx) {
    ...
    const spans = new Map(session.paragraphSpans(STORY).map((span) => [span.paraId, span.length]));
    const target = addressable(session.paragraphs(STORY)) ?? session.paragraphs(STORY).find((paragraph) => /\S/.test(paragraph.text))!;
    recorder.op('insertText:marker', () => session.insertText({ story: STORY, paraId: target.paraId, offset: spans.get(target.paraId)! }, ` ${MARKER}`));
    const table = recorder.op('insertTable', () => session.insertTable({ story: STORY, paraId: target.paraId, offset: 0 }, 2, 2));
    ...
    try {
      await worker.start();
      const opened = await worker.call<{ tablesBefore: number; tablesAfter: number; sections: number; text: string; marked: string[]; paraId: string | null; paraText: string | null }>(
        'python:open',
        OPEN,
        { original: toBase64(ctx.bytes), edited: toBase64(handed), marker: MARKER },
      ...
      if (opened.paraId === null) {
        ...
        await expect(worker.call('python:replaceUnknown', REPLACE, { paraId: 'FFFFFFFF', text: REPLACEMENT })).rejects.toThrow();
        return;
      }

      const occurrences = (session_: typeof session) => session_.storyIds().flatMap((storyId) => session_.paragraphs(storyId)).filter((paragraph) => paragraph.text === opened.paraText).length;
      const before = occurrences(session);
      expect(before).toBeGreaterThan(0);
      const replaced = await worker.call<{ paraId: string; text: string; bytes: string }>('python:replaceAndSave', REPLACE, { paraId: opened.paraId, text: REPLACEMENT });
      expect(replaced.paraId).toBe(opened.paraId);
      expect(replaced.text).toBe(REPLACEMENT);

      const returned = fromBase64(replaced.bytes);
      const reopened = await recorder.opAsync('reopen:pythonBytes', () => open(returned));
      const text = recorder
        .op('paragraphs:afterReopen', () => reopened.session.storyIds().flatMap((storyId) => reopened.session.paragraphs(storyId)))
        .map((paragraph) => paragraph.text)
        .join('\n');
      expect(text).toContain(REPLACEMENT);
      expect(text).toContain(MARKER);
      expect(occurrences(reopened.session)).toBe(before - 1);
      expect(recorder.op('storyIds:afterReopen', () => reopened.session.storyIds()).length).toBe(session.storyIds().length);
    } finally {
      worker.close();
    }
  },
...
function addressable(paragraphs: YrsParagraph[]): YrsParagraph | undefined {
  return paragraphs.find((paragraph) => /\S/.test(paragraph.text) && /^[0-9A-Fa-f]{8}$/.test(paragraph.paraId));
}
