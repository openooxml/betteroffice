...
export const editingSession: DocxScenario = {
  ...
  description: 'One editor types a word, a space and a phrase through the resident layout path, splits the paragraph, inserts a table, bolds the typed word, undoes and redoes it, then saves and reopens the document.',
  participants: ['web'],
  async run({ recorder, open }) {
    ...
    const expectTyped = (text: string) => {
      ...
      expect(session.selection()?.head).toEqual({ story: STORY, paraId: caret.paraId, offset: caret.offset + text.length });
    };
    ...
    const table = recorder.op('insertTable', () => session.insertTable({ story: STORY, paraId: typedParaId, offset: 0 }, 2, 2));
    ...
    const range = matchRange(recorder.op('searchText:typed', () => session.searchText(WORD)), typedParaId, WORD.length);
    ...
    const edited = regionLayout(editor, recorder, 'layoutDocumentWithRegions:afterEdits');
    ...
    const persisted = recorder.op('paragraphs:afterReopen', () => reopened.session.paragraphs(STORY)).find((paragraph) => paragraph.text === TYPED);
    expect(persisted).toBeDefined();
    const found = recorder.op('searchText:afterReopen', () => reopened.session.searchText(WORD));
    expect(reopened.session.selectionContext(matchRange(found, persisted!.paraId, WORD.length)).bold).toBe(true);
  },
};
