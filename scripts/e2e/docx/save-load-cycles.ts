...
import { STORY, paragraphText, regionLayout, save, typingTarget } from './context';
...
export const saveLoadCycles: DocxScenario = {
  ...
  description: 'Ten rounds of type, project, repack and reopen in a fresh session, checking every earlier marker survives and that neither the byte count nor the save and open latency grows with the round.',
  participants: ['web'],
  async run({ recorder, open }) {
    ...
    for (let cycle = 0; cycle < CYCLES; cycle += 1) {
      ...
      recorder.op('insertText:cycle', () => session.insertText({ story: STORY, paraId: target.paraId, offset: target.end }, marker));
      ...
      const text = recorder.op('paragraphs:cycle', () => editor.session.paragraphs(STORY)).map((paragraph) => paragraph.text).join('\n');
      for (let earlier = 0; earlier <= cycle; earlier += 1) expect(text).toContain(`[cycle ${earlier}]`);
    }

    const found = recorder.op('searchText:allCycles', () => editor.session.searchText('[cycle '));
