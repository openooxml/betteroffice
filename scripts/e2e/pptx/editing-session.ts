...
import { FONT_FAMILY, editStages, firstStory, historyStages, laidOutText, profiled, profiledLayout, runCount, storyText, textBoxOf } from './context';
...
export const editingSession: PptxScenario = {
  ...
  description: 'One editor inserts text, adds and moves a text box, appends a slide, deletes text and undoes it, relaying out after each change, then saves and reopens.',
  participants: ['web'],
  run({ recorder, open }) {
    ...
    const inserted = profiled(recorder, 'insertText', () => handle.insertTextProfiled(story.id, 0, MARKER), editStages);
    expect(inserted).toMatchObject({ storyId: story.id, start: 0, end: MARKER.length });
    expect(storyText(recorder.op('story:afterInsertText', () => handle.story(story.id)))).toBe(MARKER + original);
    ...
    const added = profiled(
      recorder,
      'addTextBox',
      () => handle.addTextBoxProfiled(slide.id, { name: 'E2E note', rect: BOX, text: NOTE, style: { fontSizePt: 18, fontFamily: FONT_FAMILY } }),
      editStages
    );
    ...
    const moved = profiled(recorder, 'moveShape', () => handle.moveShapeProfiled(slide.id, added.shapeId, MOVED.x, MOVED.y), editStages);
    ...
    const appended = profiled(recorder, 'insertSlide', () => handle.insertSlideProfiled(count), editStages);
    ...
    const deleted = profiled(recorder, 'deleteText', () => handle.deleteTextProfiled(story.id, 0, MARKER.length), editStages);
    expect(deleted.text).toBe(MARKER);
    expect(storyText(recorder.op('story:afterDeleteText', () => handle.story(story.id)))).toBe(original);
    ...
    expect(storyText(recorder.op('story:afterUndo', () => handle.story(story.id)))).toBe(MARKER + original);
