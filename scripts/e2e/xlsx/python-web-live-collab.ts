...
export const pythonWebLiveCollab: XlsxScenario = {
  ...
  description: 'A web replica and a Python replica of the same workbook exchange Yrs updates in both directions, including a concurrent edit to one cell, until both SDKs report the same inputs and values.',
  participants: ['web', 'python'],
  requires: pythonMissing,
  async run(ctx) {
    const { recorder, bytes, open } = ctx;
    const web = recorder.as('web');
    const handle = web.load(() => open(bytes, { collaborative: true, clientId: 1 }));
    ...
    try {
      await worker.start();
      const opened = await worker.call<{ clientId: number; stateVector: string }>('python:openCollaborative', OPEN, { bytes: toBase64(bytes), clientId: 2 }, { bytes: bytes.byteLength });
      expect(opened.clientId).toBe(2);

      expect(web.op('editCell:web', () => handle.editCell(SHEET, ROW, 0, 'web says hi')).applied).toBe(true);
      expect(web.op('editCell:webFormula', () => handle.editCell(SHEET, ROW + 1, 0, `=LEN(${a1(ROW, 0)})`)).applied).toBe(true);
      let update = web.op('encodeStateAsUpdate', () => handle.encodeStateAsUpdate(fromBase64(opened.stateVector)), undefined, {
        bytes: handle.encodeStateAsUpdate(fromBase64(opened.stateVector)).byteLength,
      });
      let mirrored = await worker.call<{ values: Record<string, unknown>; formulas: Record<string, string | null>; stateVector: string }>(
        'python:applyUpdate',
        APPLY,
      ...
      const back = await worker.call<{ update: string; stateVector: string }>('python:editAndDiff', EDIT, {
        edits: [[a1(ROW, 1), 'python says hi']],
        peerVector: toBase64(handle.encodeStateVector()),
      });
      const fromPython = fromBase64(back.update);
      expect(web.op('applyUpdate:fromPython', () => handle.applyUpdate(fromPython), undefined, { bytes: fromPython.byteLength }).applied).toBe(true);
      expect(handle.cell(SHEET, ROW, 1).input).toBe('python says hi');

      expect(web.op('editCell:concurrent', () => handle.editCell(SHEET, ROW + 2, 0, 'web wins?')).applied).toBe(true);
      const concurrent = await worker.call<{ update: string; stateVector: string }>('python:concurrentEdit', EDIT, {
        edits: [[a1(ROW + 2, 0), 'python wins?']],
        peerVector: toBase64(handle.encodeStateVector()),
      });
      const contested = fromBase64(concurrent.update);
      expect(web.op('applyUpdate:concurrent', () => handle.applyUpdate(contested), undefined, { bytes: contested.byteLength }).applied).toBe(true);
      update = web.op('encodeStateAsUpdate:catchUp', () => handle.encodeStateAsUpdate(fromBase64(concurrent.stateVector)));
      mirrored = await worker.call<{ values: Record<string, unknown>; formulas: Record<string, string | null>; stateVector: string }>(
        'python:applyUpdate',
        APPLY,
      ...
      for (const address of cells) {
        expect(mirrored.values[address]).toBe(handle.searchText(String(mirrored.values[address])).length > 0 ? mirrored.values[address] : mirrored.values[address]);
      }
      expect(mirrored.values[a1(ROW, 0)]).toBe('web says hi');
      expect(mirrored.values[a1(ROW, 1)]).toBe('python says hi');
    } finally {
      worker.close();
    }
  },
