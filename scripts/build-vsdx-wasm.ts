...
const crate = resolve(root, 'crates/vsdx-wasm');
const output = resolve(root, 'target/wasm-pack/vsdx');
const generated = resolve(root, 'packages/vsdx/src/wasm/generated');
...
const gluePath = resolve(output, 'vsdx_wasm.js');
const glue = await readFile(gluePath, 'utf8');
const fallback = "module_or_path = new URL('vsdx_wasm_bg.wasm', import.meta.url);";
if (!glue.includes(fallback)) throw new Error('wasm-pack glue fallback changed');
await writeFile(
  resolve(generated, 'vsdx_wasm.js'),
  glue.replace(fallback, "throw new Error('vsdx-wasm requires an explicit module or URL');")
);

for (const file of ['vsdx_wasm.d.ts', 'vsdx_wasm_bg.wasm', 'vsdx_wasm_bg.wasm.d.ts']) {
